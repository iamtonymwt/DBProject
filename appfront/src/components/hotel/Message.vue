...
<script>
    import LeaveMessage from "@/components/hotel/LeaveMessage";
    import ReplyMessage from "@/components/hotel/ReplyMessage";
    export default {
        name: "Message",
        ...
        methods:{
            ...
            getLeaveMessage() {
                this.$axios.post("/leavemessage/", {hotel_id: this.$store.state.hotel.id}).then(
                    resp=>{
            ...
            async getReplyMessage() {
                ...
                for (i=0;i < _this.leaveMessage.length;i++) {
                    ...
                    await this.$axios.post("/replymessage/", {leveMessage_id: id}).then(
                        resp=>{
                            if (resp && resp.status === 200) {
